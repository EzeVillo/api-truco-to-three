...
serializador de exportación. El acceso a los datos se hace consultando la tabla directamente por SQL.
...
> sin evento WebSocket nuevo, sin CLI). El único "contrato" es el esquema de la tabla, documentado en
> `data-model.md`.
...
un paquete `domain.model.gameplay`, orquestación en application (decorator), persistencia en
infrastructure.
...
7. **Serialización JSONB** de `MatchSnapshot` y `RecordedAction` reutilizando el `ObjectMapper`/patrón

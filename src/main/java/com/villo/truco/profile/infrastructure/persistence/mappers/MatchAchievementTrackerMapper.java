...
public class MatchAchievementTrackerMapper {
  ...
  public MatchAchievementTracker toDomain(final MatchAchievementTrackerJpaEntity entity) {
    ...
    final var snapshot = new MatchAchievementTrackerSnapshot(new MatchId(entity.getMatchId()),
        ...
        state.currentGameNumber(), parsePlayerSeat(state.manoSeat()),
        state.previousScorePlayerOne(), state.previousScorePlayerTwo(), state.scorePlayerOne(),
        state.scorePlayerTwo(), state.playedHandsInRound(), state.cardsPlayedInRound(),
        state.roundHadCalls(),
        state.envidoCallsInRound().stream().map(EnvidoCall::valueOf).toList(),
        parseEnvidoResponse(state.lastEnvidoResponse()),
        parsePlayerSeat(state.lastEnvidoWinnerSeat()), state.lastEnvidoPointsMano(),
        state.lastEnvidoPointsPie(), parseTrucoResponse(state.lastTrucoResponse()),
        parseTrucoCall(state.lastTrucoResponseCall()),
        parsePlayerSeat(state.lastTrucoResponderSeat()),
        parsePlayerSeat(state.lastTrucoCallerSeat()), toCard(state.lastHandCardPlayerOne()),
        toCard(state.lastHandCardPlayerTwo()), parsePlayerSeat(state.lastHandWinnerSeat()),
        parsePlayerSeat(state.lastFoldedSeat()), state.playerOnePlayedCurrentHand(),
        state.playerTwoPlayedCurrentHand(), state.trucoCalledWhenRivalHadNoCards());
  ...
  private MatchAchievementTrackerStateData toStateData(
      final MatchAchievementTrackerSnapshot snapshot) {
    ...
    return new MatchAchievementTrackerStateData(snapshot.currentGameNumber(),
        ...
        snapshot.cardsPlayedInRound(), snapshot.roundHadCalls(),
        snapshot.envidoCallsInRound().stream().map(EnvidoCall::name).toList(),
        ...
        snapshot.lastTrucoCallerSeat() != null ? snapshot.lastTrucoCallerSeat().name() : null,
        ...
        snapshot.playerOnePlayedCurrentHand(), snapshot.playerTwoPlayedCurrentHand(),
        snapshot.trucoCalledWhenRivalHadNoCards());
  }

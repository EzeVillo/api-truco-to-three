...
public final class CupPresenceEventTranslator implements CupDomainEventHandler<CupDomainEvent> {
  ...
  private static boolean isOccupancyTransition(final CupDomainEvent event) {
    ...
    return event instanceof CupPlayerJoinedEvent || event instanceof CupStartedEvent
        || event instanceof CupMatchActivatedEvent || event instanceof CupAdvancedEvent
        || event instanceof CupFinishedEvent || event instanceof CupCancelledEvent
        || event instanceof CupPlayerLeftEvent || event instanceof CupPlayerForfeitedEvent;
  }

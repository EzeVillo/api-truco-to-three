...
public record MatchStateDTO(String matchId, String status, int gamesWonPlayerOne,
    ...
    public static MatchStateDTO of(final Match match, final PlayerId requestingPlayer) {
        ...
        final var RoundStateDTO =
            match.getStatus() != MatchStatus.WAITING_FOR_PLAYERS && !match.isFinished()
                ? toRoundStateDTO(match, requestingPlayer) : null;
    ...
    private static RoundStateDTO toRoundStateDTO(final Match match,
        final PlayerId requestingPlayer) {
        ...
        final var currentTrucoCall = Optional.ofNullable(match.getCurrentTrucoCall())
            .map(Enum::name)
            .orElse(null);
        ...
        final var playedHands = match.getPlayedHands().stream().map(
            ph -> new PlayedHandDTO(CardDTO.from(ph.cardPlayerOne()),
                CardDTO.from(ph.cardPlayerTwo()),

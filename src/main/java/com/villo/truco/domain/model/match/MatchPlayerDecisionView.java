...
public record MatchPlayerDecisionView(GameContext game, TrucoContext truco, EnvidoContext envido) {
  ...
  public static MatchPlayerDecisionView empty(final int myScore, final int rivalScore) {
    ...
    return new MatchPlayerDecisionView(
        new GameContext(List.of(), myScore, rivalScore, null, 0, 0, false, false, false,
            ScoringPolicy.pointsToWinGame()),
        new TrucoContext(null, List.of(), null),
        new EnvidoContext(List.of(), List.of(), List.of(), null));
  }

  public boolean hasAvailableActions() {

    return this.game.canPlayCard()
        || this.game.canFold()
        || this.truco.availableCall() != null
        || !this.truco.availableResponses().isEmpty()
        || !this.envido.availableCalls().isEmpty()
  ...
  public record TrucoContext(TrucoCall availableCall,
                             List<TrucoResponse> availableResponses,
                             TrucoCall currentCall) {

    public TrucoContext {

      availableResponses = List.copyOf(
          Objects.requireNonNull(availableResponses));
    }
  }
  ...
  public record PendingEnvidoOutcome(int acceptedPointsIfPlayerWins,
